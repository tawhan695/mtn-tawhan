        ...
        <div class="pt-4">
            <div class="card card-info">
                                ...
                                <tbody>
                                    @foreach ($details as $item)
                                        <tr>
                                            ...
                                            <td style="width: 1px">
                                                {{ number_format(floatval($item->price) * intval($item->qty), 2, '.', ',') }}
                                            </td>
                                            <td style="width: 1px">
                                                <button type="button" class="btn btn-outline-primary"
                                                    onclick="al2{{ $item->id }}()">แก้ไข</button>
                                    ...
                                    <tr class="">
                                        ...
                                        <td style="width: 1px">{{ number_format($order->cash_totol, 2, '.', ',') }}
                                        </td>
                                        <td style="width: 1px"></td>
                                    </tr>
                                    <tr class="noBorder">
                                    ...
                                    <tr class="noBorder">
                                        ...
                                        <td style="width: 1px">{{ number_format($order->net_amount, 2, '.', ',') }}
                                        </td>
                                        <td style="width: 1px"></td>
                                    </tr>
                                </tbody>
                ...
                <div class="card-footer">
                    <form action="{{ route('return.destroy',$order) }}" method="post" id="iddelform{{ $order->id }}">
                        @csrf
                        @method('delete')
                        <button type="button" id="iddel{{ $order->id }}"
                            class="btn btn-outline-danger">ยกเลิกคำสั่งชื้อ</button>
                        <script type="text/javascript">
                            $('#iddel{{ $order->id }}').click(function() {
                                Swal.fire({
                                    title: 'ต้องการยกเลิกคำสั่งชื้อหรือไม่ ?',
                                    showDenyButton: true,
                                    showConfirmButton: false,
                                    showCancelButton: true,
                                    // confirmButtonText: `Save`,
                                    denyButtonText: `ยกเลิกคำสั่งชื้อ`,
                                }).then((result) => {
                                    /* Read more about isConfirmed, isDenied below */
                                    if (result.isDenied) {
                                        // Swal.fire('Saved!', '', 'success')
                                        $('#iddelform{{ $order->id }}').submit();
                                    }
                                })
                            })
                        </script>
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        data['data'].forEach((e, index) => {
                            ...
                            var date = yyyy + '/' + mm + '/' + dd + '  ' + todate.getHours() + ':' +
                                todate.getMinutes();
                            $('#list').append(
                                `<tr><td>` + (index += 1) + `</td>
                        <td>` + e.id + `</td>
